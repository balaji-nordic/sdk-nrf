...
static void log_event(const struct app_event_header *aeh)
{
	...
	if (event->type == APP_EVT_ERROR) {
		...
		for (int i = 0; i < event->count; i++) {
			/* data_types should contain space for the stringified type. */
			size_t max_append_len = sizeof(data_types) - strlen(data_types);
			if (i < event->count - 1) {
				/* For every type except the last one, we also need space for a comma and a
				 * space.
				 */
				max_append_len = max_append_len - 2;
			}
