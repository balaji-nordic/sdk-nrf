...
int lwm2m_codec_helpers_setup_resources(void)
{
	...
	err = lwm2m_engine_set_res_buf(LWM2M_PATH(IPSO_OBJECT_TEMP_SENSOR_ID, 0, SENSOR_UNITS_RID),
				       BME680_TEMP_UNIT, strlen(BME680_TEMP_UNIT),
				       strlen(BME680_TEMP_UNIT), LWM2M_RES_DATA_FLAG_RO);
	if (err) {
		return err;
	}
	err = lwm2m_engine_set_res_buf(LWM2M_PATH(IPSO_OBJECT_HUMIDITY_SENSOR_ID, 0,
						  SENSOR_UNITS_RID),
				       BME680_HUMID_UNIT, strlen(BME680_HUMID_UNIT),
				       strlen(BME680_HUMID_UNIT), LWM2M_RES_DATA_FLAG_RO);
	if (err) {
		return err;
	}

	err = lwm2m_engine_set_res_buf(LWM2M_PATH(IPSO_OBJECT_PRESSURE_ID, 0, SENSOR_UNITS_RID),
				       BME680_PRESSURE_UNIT, strlen(BME680_PRESSURE_UNIT),
				       strlen(BME680_PRESSURE_UNIT), LWM2M_RES_DATA_FLAG_RO);
	if (err) {
		return err;
	}
	...
	if (CONFIG_LWM2M_IPSO_PUSH_BUTTON_INSTANCE_COUNT == 2) {
		err = lwm2m_engine_set_res_buf(LWM2M_PATH(IPSO_OBJECT_PUSH_BUTTON_ID,
					       ...
					       BUTTON2_APP_NAME, strlen(BUTTON2_APP_NAME),
					       strlen(BUTTON2_APP_NAME), LWM2M_RES_DATA_FLAG_RO);
		if (err) {
			return err;
		}
...
int lwm2m_codec_helpers_set_modem_dynamic_data(struct cloud_data_modem_dynamic *modem_dynamic)
{
	...
	err = lwm2m_engine_set_res_buf(LWM2M_PATH(LWM2M_OBJECT_CONNECTIVITY_MONITORING_ID,
							0, IP_ADDRESSES, 0),
					modem_dynamic->ip, strlen(modem_dynamic->ip),
					strlen(modem_dynamic->ip), LWM2M_RES_DATA_FLAG_RO);
	if (err) {
		return err;
	}
	...
	err = lwm2m_engine_set_res_buf(LWM2M_PATH(LWM2M_OBJECT_CONNECTIVITY_MONITORING_ID,
							0, APN, 0),
					modem_dynamic->apn, strlen(modem_dynamic->apn),
					strlen(modem_dynamic->apn), LWM2M_RES_DATA_FLAG_RO);
	if (err) {
		return err;
	}
